...
int my_cb_valueETX_get(char *buffer, const struct kernel_param *kp)
{
	int res;
	printk(KERN_INFO "Returning cb_valueETX value : %d", cb_valueETX);
	res = sprintf(buffer, "%d", cb_valueETX);
	if (res != 0)
		printk(KERN_INFO "my_cb_valueETX_get called ");
...
const struct kernel_param_ops my_param_ops = 
{
        ...
        .get = &my_cb_valueETX_get, // .. and standard getter
};
